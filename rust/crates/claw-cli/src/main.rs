...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let workspace_summary = git_branch.as_deref().map_or_else(
            ...
            |branch| format!("{workspace_name} on {branch}"),
        );
        let has_claw_md = cwd
            .as_ref()
...
fn format_internal_prompt_progress_line(
    ...
    match event {
        ...
        InternalPromptProgressEvent::Complete => format!(
            "[{}] status · completed · {elapsed_seconds}s elapsed · {status} · {} steps total",
            snapshot.command_label, snapshot.step
        ),
        InternalPromptProgressEvent::Failed => format!(
            "[{}] status · failed · {elapsed_seconds}s elapsed · {status} · {}",
            snapshot.command_label,
...
fn format_tool_call_start(name: &str, input: &str) -> String {
    ...
    format!(
        "\x1b[38;5;245m╭─\x1b[1;36m {name} \x1b[38;5;245m─{border}\x1b[0m\n\x1b[38;5;245m│\x1b[0m {detail}\n\x1b[38;5;245m╰─{border}\x1b[0m"
    )
}
